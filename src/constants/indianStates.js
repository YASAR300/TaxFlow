...
/**
 * Find a state by its 2-letter ISO code or 2-digit GST code.
 * @param {string} code — e.g. "MH" or "27"
...
export function getStateByCode(code) {
  ...
  return INDIAN_STATES.find(
    s => s.code === upper || s.gstCode === upper.padStart(2, '0')
  );
}

/**
 * Find a state by its GST state code (first 2 chars of a GSTIN).
 * @param {string} gstCode — e.g. "27"
 * @returns {object|undefined}
 */
export function getStateByGSTCode(gstCode) {
  if (!gstCode) return undefined;
  const padded = String(gstCode).trim().padStart(2, '0');
  return INDIAN_STATES.find(s => s.gstCode === padded);
}
